...
export async function getMindBlowCount(paperId: string): Promise<number> {
  try {
    // Use the raw query to avoid TypeScript error with table types
    const { data, error } = await supabase
      .from('mind_blows')
      .select('count')
      .eq('paper_doi', paperId)
      .maybeSingle();
...
export async function hasUserMindBlown(paperId: string, userId: string): Promise<boolean> {
  try {
    // Use the raw query to avoid TypeScript error with table types
    const { data, error } = await supabase
...
export async function addMindBlow(paperId: string, userId: string, reason?: string): Promise<boolean> {
  try {
...
export async function removeMindBlow(paperId: string, userId: string): Promise<boolean> {
  try {
...
export async function isTopMindBlownPaper(paperId: string, threshold: number = 5): Promise<boolean> {
  try {
    const count = await getMindBlowCount(paperId);
