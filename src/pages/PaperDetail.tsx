...
const PaperDetail: React.FC = () => {
  ...
  if (loading) {
    return <div className="min-h-screen bg-white text-gray-800 p-4 flex items-center justify-center">
        <div className="loading-spinner" />
      </div>;
  }
  if (!paper) {
    return <div className="min-h-screen bg-white text-gray-800 p-4">
  ...
  const cleanAbstract = paper.abstract_org ? paper.abstract_org.includes("Abstract:") ? paper.abstract_org.split("Abstract:")[1].trim() : paper.abstract_org : "";
  return <motion.div className="min-h-screen bg-white text-gray-800" initial={{
    opacity: 0
  }} animate={{
    opacity: 1
  }} exit={{
    ...
    ease: "easeInOut"
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-lg border-b border-gray-200">
        <div className="container max-w-3xl mx-auto px-4 py-4 flex items-center">
          <Button variant="ghost" size="icon" onClick={goBack} className="mr-2 text-gray-600">
      ...
      <div className="container max-w-3xl mx-auto px-4 py-6 pt-20 pb-24">
        <div className="relative">
          ...
          {mindBlowCount > 0 && <div className="absolute top-4 right-4 z-10">
          ...
          duration: 0.4
            ...
            <HeroImageSection imageSrc={imageSrc} title={paper.ai_headline || paper.title_org} creator={paper.creator} categories={paper.category ? Array.isArray(paper.category) ? paper.category : [paper.category] : []} mindBlowCount={mindBlowCount} isTopPaper={isTopPaper} />
          </motion.div>
        </div>
        
        <div className="mt-4">
          <RegenerateImageButton paper={paper} onRegenerationStart={handleRegenerationStart} onRegenerationComplete={handleRegenerationComplete} />
        </div>
        
        <motion.div className="mb-4 flex flex-wrap gap-2 mt-6" initial={{
        opacity: 0,
        y: 10
      }} animate={{
        opacity: 1,
        ...
        delay: 0.1
          ...
          {formattedCategoryNames.map((category, idx) => <Badge key={idx} variant="outline" className="bg-gray-100 text-gray-700 border-gray-200">
              {category}
            </Badge>)}
        </motion.div>
        ...
        {paper.ai_headline && paper.title_org && paper.ai_headline !== paper.title_org && <motion.div className="mb-6" initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} transition={{
        ...
        {formattedTakeaways && formattedTakeaways.length > 0 && <motion.div className="mb-8" initial={{
        opacity: 0,
        y: 10
      }} animate={{
        opacity: 1,
        ...
        {cleanAbstract && <motion.div className="mb-8" initial={{
        opacity: 0,
        y: 10
      }} animate={{
        opacity: 1,
        ...
        <motion.div className="mt-12 border-t border-gray-200 pt-4" initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} transition={{
        delay: 0.5
          ...
          <div className="flex justify-between items-center">
            {paper.score && <div className="flex items-center">
      ...
      <DetailNavButtons paperDoi={paper.doi} onClose={goBack} />
      
      <DonationPrompt isVisible={showDonationPrompt} onClose={handleCloseDonationPrompt} onDonate={() => handleDonate(false)} onSubscribe={() => handleDonate(true)} />
